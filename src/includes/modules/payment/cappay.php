...
$payment_lang = DIR . '/languages/' . $GLOBALS['skyuc']->options ['lang'] . '/payment/cappay.php';
...
if (isset ( $set_modules ) && $set_modules == TRUE) {
	...
	/* 作者 */
	$modules [$i] ['author'] = 'beijing.com.cn';
	...
	/* 配置信息 */
	$modules [$i] ['config'] = array (array ('name' => 'cappay_account', 'type' => 'text', 'value' => '' ), array ('name' => 'cappay_key', 'type' => 'text', 'value' => '' ), array ('name' => 'cappay_currency', 'type' => 'select', 'value' => 'USD' ) );
...
class cappay {
	...
	public function get_code($order, $payment) {
        ...
        $def_url .= "<input type='submit' value='"  . $GLOBALS['skyuc']->options ['lang']['cappay_member_button'] .  "'>";
        ...
        $def_url .= "<input type='hidden' name='v_url' value='".$v_url."'>";              //支付动作完成后返回到该url，支付结果以GET方式发送
        $def_url .= "<input type='hidden' name='v_md5info' value=$result[0]>";           //订单数字指纹
        $def_url .= "<input type='submit' value='"  . $GLOBALS['skyuc']->options ['lang']['cappay_mobile_button'] .  "'>";
        ...
        $def_url .= "<input type='hidden' name='v_url' value='".$v_url."'>";              //支付动作完成后返回到该url，支付结果以GET方式发送
        $def_url .= "<input type='hidden' name='v_md5info' value=$result[0]>";           //订单数字指纹
        $def_url .= "<input type='submit' value='"  . $GLOBALS['skyuc']->options ['lang']['cappay_en_button'] .  "'>";
