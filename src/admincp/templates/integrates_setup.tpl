...
<div class="main-div">
    ...
    <form name="theForm" action="integrate.php" method="post">
        <table width="90%" id="ucenter-table">
          <tr>
            ...
            <td><input type="text" name="cfg[uc_id]" value="{$cfg.uc_id}" /><span class="notice-span" id="noticeShowSN">{$lang.ucenter_notice_id}</span></td>
          </tr>
          <tr>
            <td class="label">{$lang.ucenter_lab_key}</td>
            <td><input type="text" name="cfg[uc_key]" value="{$cfg.uc_key}" /><span class="notice-span" id="noticeShowSN">{$lang.ucenter_notice_key}</span></td>
          </tr>
          <tr>
            <td class="label">{$lang.ucenter_lab_url}</td>
            <td><input type="text" name="cfg[uc_url]" value="{$cfg.uc_url}" /><span class="notice-span" id="noticeShowSN">{$lang.ucenter_notice_url}</span></td>
          </tr>
          <tr>
            <td class="label">{$lang.ucenter_lab_ip}</td>
            <td><input type="text" name="cfg[uc_ip]" value="{$cfg.uc_ip}" /><span class="notice-span" id="noticeShowSN">{$lang.ucenter_notice_ip}</span></td>
          </tr>
          <tr>
            <td class="label">{$lang.ucenter_lab_connect}</td>
            <td><input type="radio" id="ucenter_connect_1" name="cfg[uc_connect]" value="mysql" {if $cfg.uc_connect eq "mysql"}checked="checked"{/if} /><label for="ucenter_connect_1">{$lang.ucenter_opt_database}</label><input type="radio" id="ucenter_connect_2" name="cfg[uc_connect]" value="post" {if $cfg.uc_connect neq "mysql"}checked="checked"{/if} /><label for="ucenter_connect_2">{$lang.ucenter_opt_interface}</label><span class="notice-span" id="noticeShowSN">{$lang.ucenter_notice_connect}</span></td>
          </tr>
          <tbody id="uc_db" style="display:none">
          <tr>
            <td class="label">{$lang.ucenter_lab_db_host}</td>
            <td><input type="text" name="cfg[db_host]" value="{$cfg.db_host}" /><span class="notice-span" id="noticeShowSN">{$lang.ucenter_notice_db_host}</span></td>
          </tr>
          <tr>
        ...
        <div class="button-div">
            ...
            <input type="submit" class="button primary submitButton" value="{$lang.button_force_save_config}"  />
  ...
  <table cellspacing="1" cellpadding="3" width="100%">
    ...
    <tr id="ucip" style="display:none">
        ...
        <td align="left"><input name="uc_ip" type="text" id="uc_ip"  value="" /><span class="notice-span" id="noticeShowSN">{$lang.uc_notice_ip}</span></td>
    </tr>
    <tr>
    ...
    <tr>
      <td colspan="2" align="center">
        <input type="button" class="button"  value="{$lang.button_next}" onclick="save_uc_config(0)"  />
        <input type="button" class="button"  value="{$lang.button_force_save_config}" onclick="save_uc_config(1)"  />
        <input type="reset" class="button submitButton"  value="{$lang.button_reset}"  />
        <input type="hidden" name="code" value="{$code}" />
  ...
  <table cellspacing="1" cellpadding="3" width="100%">
    ...
    <tr>
      <td class="label"><a href="javascript:showNotice('noticeShowSN');" title="{$lang.form_notice}"><img src="images/notice.gif" width="16" height="16" border="0" alt="{$lang.form_notice}"></a>{$lang.lable_db_chartset}</td>
      <td><select class="textCtrl"  name="cfg[db_charset]">{html_options options=$set_list selected=$cfg.db_charset}</select><input type="checkbox" name="cfg[is_latin1]" value="1" {if $cfg.is_latin1}checked="checked"{/if} />{$lang.lable_is_latin1}{$lang.require_field}
      <span class="notice-span" id="noticeShowSN">{$lang.notice_latin1}</span>
      </td>
    </tr>
    <tr>
    ...
    <tr>
      <td colspan="2" align="center">
        {if $save eq 0}
        <input type="submit" class="button primary submitButton" value="{$lang.button_next}"  />
        <input type="button" class="button"  value="{$lang.button_force_save_config}" onclick="saveConfig('{$lang.save_confirm}')"  />
        {else}
        <input type="submit" class="button primary submitButton" value="{$lang.button_save_config}"  />
        {/if}
        <input type="reset" class="button submitButton"  value="{$lang.button_reset}"   />
        <input type="hidden" name="code" value="{$code}" />
        <input type="hidden" name="act" value="check_config" />
        <input type="hidden" name="save" id="SKYUC_SAVE" value="{$save|default:0}">
      </td>
    </tr>
  </table>
  </form>
...
function saveConfig(str)
{
  ...
  var save = document.getElementById('SKYUC_SAVE');
  if (confirm(str))
  {
    save.value = 1;
...
var uc_1 = document.getElementById('ucenter_connect_1');
var uc_2 = document.getElementById('ucenter_connect_2');
...
if (uc_1)
{
    uc_1.onclick = function ()
    {
        ...
        {
            document.getElementById('uc_db').style.display = '';
        }
    }
}
if (uc_2)
{
    uc_2.onclick = function ()
    {
        if (this.checked === true)
        {
            document.getElementById('uc_db').style.display = 'none';
        }
    }
}
...
function showSaveResponse (res)
{
    if (res.error !== 0)
    {
        if (res.error == 2)
        {
            document.getElementById("ucip").style.display = '';
        }
        document.getElementById("ucfounderpwnotice").innerHTML= res.message;
    }
    else
    {
        var frm = document.forms['theForm'];
