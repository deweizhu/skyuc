...
<table>
    ...
    {if $key eq $rule_index}
     <tr>
      <td style="padding-left:8px;">
        <select class="textCtrl"  name="rule_index">
      ...
      <td align="center"><input type="hidden" name="old_rule_index" value="{$rule_index}" /><input type="submit" class="button primary submitButton" value="{$lang.button_submit}" /> <a href="integrate.php?act=edit_points&rule_index={$key}">{$lang.remove}</a></td>
    </tr>
    {else}
    <tr>
      <td style="padding-left:8px;">
  ...
  {if $allow_add}
   <tr id="addRow" style="display:none">
    ...
    <td align="center"><input type="submit" class="button primary submitButton" value="{$lang.button_submit}" /> <a href="javascript:;" onclick="removeRow()">{$lang.remove}</a></td>
  </tr>
  <tr>
    <td colspan="3" align="center"><input type="button" class="button"  name="addRule" value="{$lang.add_rule}" onclick="addRow()" /></td>
  </tr>
  {else}
  <tr>
    <td colspan="3" align="center"><input type="button" class="button"  name="addRule" value="{$lang.add_rule}" disabled="disabled" /></td>
  </tr>
  {/if}
</table>
</form>
